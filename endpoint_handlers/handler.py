    ...
    def invoke(self, request: Request, values: dict, settings: dict) -> Response:
        ...
        try:
            ...
            if method == "POST" and path == "/repositories":
                from endpoint_handlers.repositories import create_repository, CreateRepositoryRequest
                import uuid
                ...
                auto_sync = request_data.get("auto_sync", False) if "auto_sync" in request_data else (settings.get("auto_sync", False) if isinstance(settings.get("auto_sync"), bool) else False)
                sync_interval = request_data.get("sync_interval", 60) if "sync_interval" in request_data else int(settings.get("sync_interval", 60)) if settings.get("sync_interval") else 60
                ...
                if not repo_url:
                    return Response(
                        json.dumps({"error": "Repository URL is required. Please configure it in plugin settings or provide it in the request."}),
                        status=400,
