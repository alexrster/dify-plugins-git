...
class ExportWorkflowTool(Tool):
    ...
    def _invoke(self, tool_parameters: dict[str, Any]):
        ...
        if config.auth_type != "none" and config.credentials:
            ...
            if isinstance(config.credentials, dict) and "encrypted" in config.credentials:
                try:
                    ...
                    yield self.create_text_message(f"❌ Failed to decrypt credentials: {str(e)}")
                    return
        ...
        try:
            ...
            try:
                ...
                if isinstance(workflow_data, str):
                    if not workflow_data.strip():
                        yield self.create_text_message(f"❌ Empty workflow data returned.\\n" f"App ID: {app_id}")
                        return
                    try:
            ...
            # Commit changes
            author = {"name": self.runtime.user_id or "Dify User", "email": f"{self.runtime.user_id or 'user'}@dify.local"}
