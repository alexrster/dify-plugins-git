...
async def export_workflow(request: ExportWorkflowRequest):
    ...
    try:
        result = await sync_service.export_workflow(
            config,
            request.workflow_id,
            request.file_naming
        )
...
async def export_application(request: ExportApplicationRequest):
    ...
    try:
        result = await sync_service.export_application(
            config,
            request.app_id,
            request.file_naming
        )
...
async def import_workflow(request: ImportWorkflowRequest):
    ...
    try:
        result = await sync_service.import_workflow(
            config,
            request.file_path,
            request.auto_merge
        )
...
async def import_application(request: ImportApplicationRequest):
    ...
    try:
        result = await sync_service.import_application(
            config,
            request.file_path,
            request.auto_merge
        )
...
async def get_sync_status(repository_id: str):
    ...
    if sync_state:
        ...
        return {
            ...
            "status": "pending",
            "message": "No sync state available"
        }
